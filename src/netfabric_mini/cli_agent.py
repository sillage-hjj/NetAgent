...
from netfabric_mini.agent.approvals import approve_request, list_pending_approvals, reject_request
from netfabric_mini.agent.planner import classify_question
from netfabric_mini.agent.report_renderer import render_agent_report_json, render_agent_report_markdown, render_agent_report_text
from netfabric_mini.agent.run_store import list_runs, list_tool_calls
...
from netfabric_mini.db import connect_db, init_sim_db
from netfabric_mini.evals.runner import run_all_agent_evals
from netfabric_mini.evals.scenarios import prepare_agent_scenario_db, scenario_question
from netfabric_mini.llm.config import DEFAULT_OPENAI_MODEL, LLMProviderConfig, load_llm_config, redacted_diagnostics, resolve_provider
...
def chat_command(
    ...
    conn = connect_db(db)
    typer.echo("NetFabric Mini agent chat. Type 'exit' to stop.")
    while True:
